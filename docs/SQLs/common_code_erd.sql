refer : common_code_erd url 추가 필요

-- commond code erd 

-- erd 미리보기 code 가져와서 table 등 create
...
-- join
SELECT USERS.*, DP.FK_COMMON_CODES AS FK_ADDRESS, CC.NAME, CC_GENDER.NAME
FROM USERS
	JOIN DELIVERY_PLACES AS DP ON DP.PK_USERS = USERS.PK_USERS
    JOIN COMMON_CODES AS CC ON CC.FK_COMMON_CODES = DP.FK_COMMON_CODES
    JOIN COMMON_CODES CC_GENDER ON CC_GENDER.FK_COMMON_CODES = USERS.FK_COMMON_CODES
;